...
public class E2E_Test_Case extends TestBase {
	...
	public void insurance_Calculate(Object obj1) throws Exception {
		...
		Assert.assertEquals(price_selection.getPriceForSelectedOption("Gold"), testData.get("PriceValidation_Gold"), "Pricing for gold plan is not matching.");
		Assert.assertEquals(price_selection.getPriceForSelectedOption("Silver"), testData.get("PriceValidation_Silver"), "Pricing for silver plan is not matching.");
		Assert.assertEquals(price_selection.getPriceForSelectedOption("Platinum"), testData.get("PriceValidation_Platinum"), "Pricing for platimum plan is not matching.");
		Assert.assertEquals(price_selection.getPriceForSelectedOption("Ultimate"), testData.get("PriceValidation_Ultimate"), "Pricing for ultimate plan is not matching.");
